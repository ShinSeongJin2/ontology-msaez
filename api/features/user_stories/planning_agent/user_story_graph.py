...
This workflow generates a human-reviewable plan for adding a new user story:
1) Analyze the story intent
2) Find best matching BC (or propose a new one)
3) Generate related objects (Aggregate / Command / Event)
"""
...
def generate_objects_node(state: UserStoryPlanningState) -> Dict[str, Any]:
    ...
    related_text = "\n".join(
        [f"- {obj.get('type', 'Unknown')}: {obj.get('name', '?')}" for obj in state.related_objects]
